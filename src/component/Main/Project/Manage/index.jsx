...
const ManageModule = (props) => {
  ...
  useEffect(() => {
    setAssignedEmployees(project.projectDetails.assignedEmployees)
  }, [project.projectDetails.assignedEmployees])
  ...
  const handleSelect = (selectedList, selectedItem) => {
    ...
    setSelectedEmployees(selectedList.map(a => {
      return {
        id: a.id,
        date: dobFormat
      }
      ...
      {Object.keys(projectDetails).length > 0 ? (
        <>
          ...
          <Row className="mx-2 mb-2">
            ...
            <Col xs={12} sm={8} className="my-0">
              <div
                className={`fw-normal text-muted ${projectDetails.project_description && "border"
                  } p-2 mb-2`}
        ...
        <h5 className="border py-3 text-center">{isLoading ? 'Loading...' : 'No Data'}</h5>
      )}
    </MainBody>
  );
};
