...
export const loginUser = userData => (dispatch /* , getState */) => {
  return axiosInstance('post', 'auth/login', userData)
    ...
    .then(res => {
      ...
      cookie.set("_wus_BJK", JSON.stringify(token), { expires: 7 });
      return res;
    })
    .catch(err => {
      return err.response.data;
...
export const usersDynamicUpdate =
  (elementId, fieldName, newValue) => (dispatch /* , getState */) => {
    ...
    return axiosInstance('put', 'auth/dynamic', userData).then(res => {
        if(res.success){
          return axiosInstance('get', `auth/${userData.usersId}`)
          .then(res => res.data)
          .then(res => {
            dispatch({
            ...
            dispatch({ type: authUserActionType.AUTH_SUCCESS, data: res.data });
